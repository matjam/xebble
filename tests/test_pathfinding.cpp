...
static float open_cost(IVec2 /*from*/, IVec2 /*to*/) {
    return 1.0f;
}
...
TEST(FindPath, WallBlocking) {
    ...
    for (auto& p : path)
        EXPECT_FALSE(walls.in_bounds(p) && walls[p])
            << "Path passes through wall at " << p.x << "," << p.y;
}
