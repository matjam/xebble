...
class ProcgenSystem : public xebble::System {
    ...
    found:
        auto reach = xebble::flood_fill(seed_pos, map_, [&](xebble::IVec2 p) { return !map_[p]; });
        gs.floor_count = 0;
        for (auto v : map_)
            gs.floor_count += (v ? 0 : 1);
...
public:
    ...
    void update(xebble::World& world, float) override {
        ...
        for (const auto& ev : world.resource<xebble::EventQueue>().events) {
            ...
            if (k == xebble::Key::C) {
                gs.mode = GenMode::Cellular;
                regen = true;
            }
            if (k == xebble::Key::D) {
                gs.mode = GenMode::Drunkard;
                regen = true;
            }
            if (k == xebble::Key::R) {
                gs.mode = GenMode::Rooms;
                regen = true;
            }
        }
        ...
        ui.panel("hud", {.anchor = xebble::Anchor::Bottom, .size = {1.0f, 48}}, [&](auto& p) {
            ...
            }
            {
                auto s = std::format("Floor cells: {:d}  Reachable: {:d}  "
                                     ...
                                     gs.floor_count > 0 ? gs.reachable * 100 / gs.floor_count : 0);
