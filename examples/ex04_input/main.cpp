...
public:
    ...
    void update(xebble::World& world, float dt) override {
        ...
        for (const auto& e : eq.events) {
            if (e.type == xebble::EventType::KeyPress || e.type == xebble::EventType::KeyRepeat) {
                switch (e.key().key) {
                case xebble::Key::W:
                case xebble::Key::Up:
                    dy -= 1;
                ...
                if (e.type == xebble::EventType::KeyPress) {
                    ...
                    state.alt = m.alt;
                    state.last_key =
                        std::format("key={} shift={} ctrl={} alt={}", static_cast<int>(e.key().key),
                                    m.shift ? "Y" : "N", m.ctrl ? "Y" : "N", m.alt ? "Y" : "N");
                }
            }
        ...
        uint8_t alpha =
            static_cast<uint8_t>(std::clamp(128.0f + state.scroll_y * 10.0f, 30.0f, 255.0f));
        spr.tint.a = alpha;
    }
    ...
    void draw(xebble::World& world, xebble::Renderer& renderer) override {
        ...
        ui.panel("hud", {.anchor = xebble::Anchor::Bottom, .size = {1.0f, 48}}, [&](auto& p) {
            p.text(u8"ex04 \u2014 Input  |  WASD/Arrows move  Left-click teleports  Scroll = alpha "
                   u8" Esc quit",
                   {.color = {220, 220, 220}});
            {
