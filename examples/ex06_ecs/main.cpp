  ...
  void update(xebble::World &world, float dt) override {
    ...
    for (const auto &ev : world.resource<xebble::EventQueue>().events) {
      ...
      if (key == xebble::Key::Delete || key == xebble::Key::Backspace) {
        ...
        world.each<Health, Velocity>(
            [&](xebble::Entity e, Health &, Velocity &) {
    ...
    if (stats.spawn_timer >= 0.3f) {
      ...
      world.build_entity()
          .with(xebble::Position{304.0f, 164.0f})
          .with(xebble::Sprite{
              &*sheet_, pixel_atlas::TILE_YELLOW, 1.0f, {220, 220, 50, 200}})
    ...
    world.each<Lifetime>([&](xebble::Entity e, Lifetime &lt) {
      ...
      world.get<xebble::Sprite>(e).tint.a =
          static_cast<uint8_t>(alpha * 200.0f);
             ...
             [&](auto &p) {
                      ...
                      {.color = {220, 220, 220}});
               {
                 auto s = std::format("Entities alive: {:5d}  |  "
                                      ...
                                      stats.entities_alive, stats.bulk_spawned,
                                      stats.bulk_removed, stats.particles_born,
                                      stats.particles_died);
...
int main() {
  ...
  return xebble::run(
      std::move(world),
      {
          .window = {.title = "ex06 — ECS Deep Dive",
                     .width = 1280,
                     .height = 720},
