...
struct Lifetime  { float remaining = 0.0f; };  // particle expires
...
class ECSSystem : public xebble::System {
    ...
    static constexpr uint32_t bulk_tiles_[6] = {
        pixel_atlas::TILE_CYAN,  pixel_atlas::TILE_GREEN,
        pixel_atlas::TILE_BLUE,  pixel_atlas::TILE_MAGENTA,
        pixel_atlas::TILE_ORANGE,pixel_atlas::TILE_FLOOR,
    };
...
public:
    void init(xebble::World& world) override {
        ...
        // Register custom component types (Position and Sprite are auto-registered).
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            float r     = 80.0f;
            world.build_entity()
                .with(xebble::Position{304.0f + r * std::cos(angle),
                                       164.0f + r * std::sin(angle)})
                .with(xebble::Sprite{&*sheet_, pixel_atlas::TILE_RED, 2.0f})
                .with(Velocity{std::cos(angle + 1.5f) * 40.0f,
                               std::sin(angle + 1.5f) * 40.0f})
                .with(Health{3, 3})
                .build();
        }
    ...
    void update(xebble::World& world, float dt) override {
        ...
        for (const auto& ev : world.resource<xebble::EventQueue>().events) {
            ...
            if (key == xebble::Key::Escape) std::exit(0);

            if (key == xebble::Key::Space) {
                // Spawn 1000 entities with random positions and velocities.
                for (int i = 0; i < 1000; ++i) {
                    ...
                    world.build_entity()
                        ...
                        .with(Velocity{std::cos(angle) * speed,
                                       std::sin(angle) * speed})
                        .with(Health{1, 1})
                        .build();
                }
                stats.bulk_spawned   += 1000;
                stats.entities_alive += 1000;
            }

            if (key == xebble::Key::Delete || key == xebble::Key::Backspace) {
                // Remove up to 1000 entities that have Health (bulk entities).
                // Skip the player by checking for Velocity (player has none).
                int removed = 0;
                std::vector<xebble::Entity> to_kill;
                world.each<Health, Velocity>([&](xebble::Entity e, Health&, Velocity&) {
                    if (removed < 1000) { to_kill.push_back(e); ++removed; }
                });
        ...
        world.each<xebble::Position, Velocity>([&](xebble::Entity, xebble::Position& pos, Velocity& vel) {
            pos.x += vel.vx * dt;
            pos.y += vel.vy * dt;
            // Wrap around the virtual screen (640x360).
            if (pos.x < 0)   pos.x += 640.0f;
            if (pos.x > 640) pos.x -= 640.0f;
            if (pos.y < 0)   pos.y += 360.0f;
            if (pos.y > 360) pos.y -= 360.0f;
        });
        ...
        if (stats.spawn_timer >= 0.3f) {
            ...
            world.build_entity()
                .with(xebble::Position{304.0f, 164.0f})
                .with(xebble::Sprite{&*sheet_, pixel_atlas::TILE_YELLOW, 1.0f,
                                     {220, 220, 50, 200}})
    ...
    void draw(xebble::World& world, xebble::Renderer&) override {
        ...
        ui.panel("hud", {.anchor = xebble::Anchor::Bottom, .size = {1.0f, 50}},
            [&](auto& p) {
                p.text(u8"ex06 \u2014 ECS Deep Dive",
                       {.color = {220, 220, 220}});
                ...
                p.text(u8"[Space] Spawn 1000 entities   [Del/Bksp] Remove 1000 entities   [Esc] Quit",
                       {.color = {180, 180, 100}});
            });
    }
};
