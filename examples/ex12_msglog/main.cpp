...
public:
    ...
    void update(xebble::World& world, float dt) override {
        ...
        for (const auto& ev : world.resource<xebble::EventQueue>().events) {
            ...
            if (k == xebble::Key::C)
                ls.log.push(u8"You strike the enemy for 7 damage.", {220, 200, 200}, "combat");
            if (k == xebble::Key::L)
                ls.log.push(u8"You find a healing potion.", {100, 220, 100}, "loot");
            if (k == xebble::Key::S)
                ls.log.push(u8"You descend deeper into the dungeon.", {180, 140, 255}, "story");
        ...
        if (ls.auto_spam) {
            ...
            if (ls.auto_timer >= 0.4f) {
                ls.auto_timer = 0.0f;
                ++ls.spam_counter;
                const std::u8string msgs[] = {u8"You miss!", u8"Critical hit!", u8"The orc growls.",
                                              u8"Gold clatters."};
    ...
    void draw(xebble::World& world, xebble::Renderer& renderer) override {
        ...
        // Main log panel.
        ui.panel("log", {.anchor = xebble::Anchor::Left, .size = {300, 260}, .offset = {10, 0}},
                 [&](auto& p) {
                     p.text(u8"Message Log", {.color = {220, 220, 100}});
                     const char* filter_label =
                         ls.filter_cat.empty() ? "all" : ls.filter_cat.c_str();
                     {
                         auto s = std::format("Filter: {:s}  |  {:d}/{:d} msgs", filter_label,
        ...
        // Controls panel.
        ui.panel("controls",
                 {.anchor = xebble::Anchor::Right, .size = {280, 220}, .offset = {-10, 0}},
                 [&](auto& p) {
