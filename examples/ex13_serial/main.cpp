...
public:
    ...
    void update(xebble::World& world, float) override {
        ...
        for (const auto& ev : world.resource<xebble::EventQueue>().events) {
            ...
            if (k == xebble::Key::S) {
                ...
                state.status = std::format("Snapshot: {:d} bytes, {:d} entities", state.blob.size(),
                                           state.entity_count);
            }
            ...
            if (k == xebble::Key::N) {
                ...
                state.entity_count = count_entities(world);
                state.status =
                    std::format("Now have {:d} entities (pre-restore)", state.entity_count);
            }
            ...
            if (k == xebble::Key::X) {
                ...
                for (auto e : to_kill)
                    world.destroy(e);
                state.entity_count = count_entities(world);
                state.status = std::format("Killed all HP entities — now {:d}", state.entity_count);
            }
        }
    }
    ...
    void draw(xebble::World& world, xebble::Renderer& renderer) override {
        ...
        std::vector<Row> rows;
        world.each<HP, Gold, SerialPos>([&](xebble::Entity, HP& hp, Gold& g, SerialPos& pos) {
        ...
        ui.panel(
            "serial_main", {.anchor = xebble::Anchor::Center, .size = {540, 280}}, [&](auto& p) {
        ...
        ui.panel("hud", {.anchor = xebble::Anchor::Bottom, .size = {1.0f, 20}}, [](auto& p) {
            p.text(u8"ex13 \u2014 Serialization  |  [Esc] Quit", {.color = {200, 200, 200}});
        });
    ...
    void spawn_entities(xebble::World& world) {
        ...
        for (int i = 0; i < 4; ++i) {
            world.build_entity()
                ...
                .with(xebble::Position{rng.range(10, 620) * 1.0f, rng.range(10, 340) * 1.0f})
