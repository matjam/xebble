...
public:
    ...
    void update(xebble::World& world, float dt) override {
        ...
        for (const auto& ev : world.resource<xebble::EventQueue>().events) {
            if (ev.type == xebble::EventType::KeyPress && ev.key().key == xebble::Key::Escape)
                std::exit(0);
        }
    ...
    void draw(xebble::World& world, xebble::Renderer& renderer) override {
        ...
        ui.panel("hud", {.anchor = xebble::Anchor::Bottom, .size = {1.0f, 34}}, [&](auto& p) {
            {
                auto s = std::format("ex03  t={:.1f}s  entities={:d}  spawned={:d}  despawned={:d}",
                                     time_, entity_count_, spawn_total_, despawn_total_);
                p.text(std::u8string(s.begin(), s.end()), {.color = {220, 220, 220}});
            }
            p.text(u8"Boids=200  Parallax=3 layers  Particles  Atlas ticker  |  [Esc] Quit",
                   {.color = {150, 150, 150}});
        });
...
private:
    ...
    void build_boids(xebble::World& world) {
        const uint32_t boid_tiles[] = {
            pixel_atlas::TILE_RED,     pixel_atlas::TILE_GREEN,  pixel_atlas::TILE_CYAN,
            pixel_atlas::TILE_MAGENTA, pixel_atlas::TILE_ORANGE, pixel_atlas::TILE_YELLOW,
        };
    ...
    void tick_boids(xebble::World& world, float dt) {
        ...
        for (size_t i = 0; i < boid_cache_.size(); ++i) {
            ...
            if (ali_n) {
                ...
                ali_x -= b.vx / BOID_SPEED;
                ali_y -= b.vy / BOID_SPEED;
                fx += ali_x * ALI_WEIGHT;
                fy += ali_y * ALI_WEIGHT;
            }
            if (coh_n) {
            ...
            float margin = 40.0f;
            if (b.x < margin)
                fx += (margin - b.x) / margin * 3.0f;
            if (b.x > VW - margin)
                fx -= (b.x - (VW - margin)) / margin * 3.0f;
            if (b.y < margin)
                fy += (margin - b.y) / margin * 3.0f;
            if (b.y > VH - margin)
                fy -= (b.y - (VH - margin)) / margin * 3.0f;
        ...
        world.each<Boid, xebble::Position, Vel, xebble::Sprite>(
            [&](xebble::Entity, Boid& boid, xebble::Position& pos, Vel& vel, xebble::Sprite& spr) {
                ...
                if (boid.flash_timer > 0.0f) {
                    ...
                    float hue =
                        std::fmod(boid.angle / (2.0f * std::numbers::pi_v<float>)+1.0f, 1.0f);
    ...
    void tick_particles(xebble::World& world, float dt) {
        ...
        world.each<Particle, xebble::Position, Vel, xebble::Sprite>(
            [&](xebble::Entity e, Particle& p, xebble::Position& pos, Vel& vel,
                xebble::Sprite& spr) {
                p.life -= dt;
                if (p.life <= 0.0f) {
                    dead.push_back(e);
    ...
    static xebble::Color hue_to_rgb(float h) {
        ...
        switch (hi) {
        case 0:
            return {255, b(f), 0, 255};
        case 1:
            return {b(1 - f), 255, 0, 255};
        case 2:
            return {0, 255, b(f), 255};
        case 3:
            return {0, b(1 - f), 255, 255};
        case 4:
            return {b(f), 0, 255, 255};
        default:
            return {255, 0, b(1 - f), 255};
        }
    }
};
