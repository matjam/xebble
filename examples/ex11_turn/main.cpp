...
public:
    ...
    void update(xebble::World& world, float dt) override {
        ...
        for (const auto& ev : world.resource<xebble::EventQueue>().events) {
            ...
            if (k == xebble::Key::Space)  step(ts);
            if (k == xebble::Key::A) { ts.mode = Mode::Alternating; reset(ts); }
            if (k == xebble::Key::E) { ts.mode = Mode::Energy;       reset(ts); }
            if (k == xebble::Key::I) { ts.mode = Mode::Initiative;   reset(ts); }
            if (k == xebble::Key::R) { ts.auto_run = !ts.auto_run;   }
        }
    ...
    void draw(xebble::World& world, xebble::Renderer& renderer) override {
        ...
        const char* mode_str = [&] {
            switch (ts.mode) {
                case Mode::Alternating: return "Alternating";
                case Mode::Energy:      return "Energy";
                case Mode::Initiative:  return "Initiative";
            }
            return "?";
        }();

        ui.panel("turn_main", {.anchor = xebble::Anchor::Center, .size = {540, 260}},
            [&](auto& p) {
                ...
                for (size_t i = 0; i < ts.actors.size(); ++i) {
                    ...
                    if (ts.mode == Mode::Initiative) {
                        for (auto& e : ts.init_sched.entries())
                            if (e.id == i) extra = std::format(" [init:{:2d}]", e.initiative);
                    }
                    { auto s = std::format("{:6s} spd={:2d}{:s} acts={:3d}",
                                       a.name, a.speed, extra, a.actions_taken);
...
private:
    void reset(TurnState& ts) {
        for (auto& a : ts.actors) a.actions_taken = 0;
        ts.tick_count = 0;
        ts.log.clear();
        ...
        switch (ts.mode) {
            case Mode::Alternating: {
                ...
                if (s.is_player_turn()) {
                    ...
                    ts.push_log(std::format("Turn {:d}: Player acts (round {:d})",
                                            ts.tick_count, (int)s.turn()));
                    s.end_player_turn();
                } else {
                    for (size_t i = 1; i < ts.actors.size(); ++i) {
                        ts.actors[i].actions_taken++;
                        ts.push_log(std::format("Turn {:d}: {:s} acts",
                                                ts.tick_count, ts.actors[i].name));
                    }
                    s.end_monster_turn();
                }
                break;
            ...
            case Mode::Energy: {
                ...
                while (s.has_ready()) {
                    ...
                    ts.push_log(std::format("Turn {:d}: {:s} acts",
                                            ts.tick_count, ts.actors[id].name));
                    s.end_turn(id);
                }
                break;
            }
            case Mode::Initiative: {
                ...
                if (!s.empty()) {
                    ...
                    ts.actors[id].actions_taken++;
                    ts.push_log(std::format("Turn {:d}: {:s} acts (init {:d})",
                                            ts.tick_count, ts.actors[id].name,
                                            s.current_initiative()));
                    s.advance();
                }
                break;
            }
