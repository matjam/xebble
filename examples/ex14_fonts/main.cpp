...
// Printable ASCII
static constexpr std::u8string_view ASCII_SAMPLE =
    u8"The quick brown fox jumps over the lazy dog.";

// Latin extended (é à ü ñ ç ø)
static constexpr std::u8string_view LATIN_EXT =
    u8"caf\u00E9 na\u00EFve r\u00E9sum\u00E9 "
    u8"\u00E9\u00E0\u00FC\u00F1\u00E7\u00F8";

// Box-drawing (┌─────┐) + block elements (█▓▒░▖▗▘▙)
static constexpr std::u8string_view BOX_SAMPLE =
    u8"\u250C\u2500\u2500\u2500\u2500\u2500\u2510 "
    u8"\u2588\u2593\u2592\u2591"
    u8"\u2596\u2597\u2598\u2599";

// Arrows (←↑→↓), maths (×÷±≠≤≥), suits (★♦♣♠♥)
static constexpr std::u8string_view SYMBOL_SAMPLE =
    u8"\u2190\u2191\u2192\u2193  "
    u8"\u00D7\u00F7\u00B1\u2260\u2264\u2265  "
    u8"\u2605\u2666\u2663\u2660\u2665";
...
public:
    void init(xebble::World& world) override {
        ...
        if (f64) petme64_ = std::make_unique<xebble::BitmapFont>(std::move(*f64));

        auto f642y = xebble::embedded_fonts::petme642y::create(ctx);
        if (f642y) petme642y_ = std::make_unique<xebble::BitmapFont>(std::move(*f642y));

        auto fberk = xebble::embedded_fonts::berkelium64::create(ctx);
        if (fberk) berkelium64_ = std::make_unique<xebble::Font>(std::move(*fberk));
        ...
        if (petme64_)    theme_petme64_.font   = petme64_.get();
        if (petme642y_)  theme_petme642y_.font  = petme642y_.get();
        if (berkelium64_) theme_berkelium_.font  = berkelium64_.get();
    }
    ...
    void draw(xebble::World& world, xebble::Renderer&) override {
        ...
        ui.panel("title", {
            ...
            p.text(u8"ex14 -- Font Showcase  |  PetMe64 / PetMe642Y / Berkelium64  |  [Esc] Quit",
                   {.color = {255, 220, 80}});
        });
...
int main() {
    ...
    return xebble::run(std::move(world), {
        .window   = {.title  = "ex14 -- Font Showcase",
                     .width  = 1280,
                     .height = 720},
