...
class UISystem : public xebble::System {
    const std::vector<std::u8string> LIST_ITEMS = {
        u8"Option Alpha",   u8"Option Beta", u8"Option Gamma", u8"Option Delta",
        u8"Option Epsilon", u8"Option Zeta", u8"Option Eta",   u8"Option Theta",
    };
    ...
    void draw(xebble::World& world, xebble::Renderer& renderer) override {
        ...
        ui.panel("list_panel",
                 ...
                 [&](auto& p) {
                     ...
                     p.list("my_list", LIST_ITEMS, state.selected_item, {.visible_rows = 6});
        ...
        ui.panel("right",
                 ...
                 [&](auto& p) {
                     ...
                     p.horizontal([&](auto& row) {
                         if (row.button(u8"A"))
                             state.button_clicks += 10;
                         if (row.button(u8"B"))
                             state.button_clicks += 100;
                         if (row.button(u8"C"))
                             state.button_clicks -= 5;
                     });
                 });
        ...
        if (state.show_stats) {
            ui.panel(
                ...
                [&](auto& p) {
                    {
                        auto s = std::format(
                            "ex05 \u2014 UI Demo  |  Clicks: {}  VSync: {}  Fullscreen: {}",
                            state.button_clicks, state.vsync ? "on" : "off",
                            state.fullscreen ? "yes" : "no");
                        p.text(std::u8string(s.begin(), s.end()), {.color = {220, 220, 220}});
                    }
                    p.text(
                        u8"All widgets: text / button / checkbox / list / text_input / horizontal",
                        {.color = {160, 160, 160}});
                });
        }
