        ...
        const CostFn& cost_fn)
{
    ...
    auto idx = [width](IVec2 p) { return p.y * width + p.x; };
    ...
    auto heuristic = [&](IVec2 p) -> float {
        return static_cast<float>(std::max(std::abs(p.x - goal.x),
                                           std::abs(p.y - goal.y)));
    };
    ...
    while (!open.empty()) {
        ...
        float cur_g = g[ci];
        if (f > cur_g + heuristic(cur) + 1e-6f) continue;
        ...
        const CostFn& cost_fn)
{
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (!in_bounds(nb)) continue;
        if (cost_fn(pos, nb) < 0.0f) continue;
        float v = dmap[nb];
        if (v < best_v) { best_v = v; best = nb; }
    }
    return best;
}
