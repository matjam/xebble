...
public:
    ...
    constexpr explicit Iterator(const char8_t* p, const char8_t* end) noexcept : p_(p), end_(end) {
        decode();
    }
...
private:
    ...
    constexpr void decode() noexcept {
        ...
        if (b0 < 0x80u) {
            ...
            if (p_ + 4 <= end_ && cont(p_[1]) && cont(p_[2]) && cont(p_[3])) {
                cp_ = ((b0 & 0x07u) << 18) | (cont_bits(p_[1]) << 12) | (cont_bits(p_[2]) << 6) |
                      cont_bits(p_[3]);
                next_ = p_ + 4;
            } else {
                cp_ = 0xFFFDu;
    ...
    static constexpr bool cont(char8_t b) noexcept {
        return (static_cast<uint8_t>(b) & 0xC0u) == 0x80u;
    }
    static constexpr uint32_t cont_bits(char8_t b) noexcept {
        return static_cast<uint32_t>(static_cast<uint8_t>(b) & 0x3Fu);
    }
};
