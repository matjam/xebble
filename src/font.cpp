...
std::optional<uint32_t> BitmapFontData::glyph_index(uint32_t codepoint) const {
    ...
    if (it == codepoint_map_.end()) return std::nullopt;
    return it->second;
}
...
BitmapFont::load_pcf(vk::Context& ctx, const std::filesystem::path& path)
{
    ...
    if (!mt) return std::unexpected(Error{"PCF: no METRICS table"});
    auto metrics_result = pcf::read_metrics_table(r, *mt);
    if (!metrics_result) return std::unexpected(metrics_result.error());
    ...
    for (size_t gi = 0; gi < num_glyphs; ++gi) {
        ...
        uint32_t row_stride = (((static_cast<uint32_t>(bm_w) + 7) / 8)
                               + binfo.pad_bytes - 1) / binfo.pad_bytes * binfo.pad_bytes;
        ...
        for (int16_t row = 0; row < bm_h; ++row) {
            for (int16_t col = 0; col < bm_w; ++col) {
                size_t byte_idx = glyph_off
                    + static_cast<size_t>(row) * row_stride
                    + static_cast<size_t>(col) / 8;
                if (byte_idx >= buf.size()) continue;
                uint8_t byte = buf[byte_idx];
                if (!binfo.msbit_first) byte = pcf::reverse_bits(byte);
                if (!((byte >> (7 - (col % 8))) & 1)) continue;
                uint32_t px = ax + static_cast<uint32_t>(col);
                uint32_t py = ay + static_cast<uint32_t>(row);
                if (px >= atlas_w || py >= atlas_h) continue;
    ...
    if (!et) return std::unexpected(Error{"PCF: no BDF_ENCODINGS table"});
    ...
    std::u8string_view charset)
{
    ...
    font.impl_ = std::make_unique<Impl>(
        std::move(*sheet), BitmapFontData(glyph_width, glyph_height, charset));
    return font;
}
...
std::expected<BitmapFont, Error> BitmapFont::from_data(
    ...
    uint32_t pixel_size)
{
    ...
    for (int i = 0; i < CHAR_COUNT; ++i) {
        if (FT_Load_Char(face, static_cast<char>(FIRST_CHAR + i), FT_LOAD_RENDER)) continue;
        auto& g  = face->glyph;
        auto& bm = bitmaps[i];
    ...
    uint32_t atlas_w = 1;
    while (atlas_w < total_width) atlas_w *= 2;
    ...
    uint32_t row_h = max_height + 1, rows_needed = 1;
    {
        uint32_t cx = 0;
        for (int i = 0; i < CHAR_COUNT; ++i) {
            if (cx + bitmaps[i].width + 1 > atlas_w) { cx = 0; ++rows_needed; }
            cx += bitmaps[i].width + 1;
        }
    }
    uint32_t atlas_h = 1;
    while (atlas_h < rows_needed * row_h) atlas_h *= 2;
    ...
    for (int i = 0; i < CHAR_COUNT; ++i) {
        auto& bm = bitmaps[i];
        if (cx + bm.width + 1 > atlas_w) { cx = 0; cy += row_h; }
        ...
        bm.metrics.uv = Rect{
            .x = static_cast<float>(cx) / aw, .y = static_cast<float>(cy) / ah,
            .w = static_cast<float>(bm.width) / aw, .h = static_cast<float>(bm.height) / ah,
        };
    ...
    if (cell_h > line_h) line_h = cell_h;
    FT_Done_Face(face);
    FT_Done_FreeType(ft);
...
std::expected<BitmapFont, Error>
BitmapFont::load_bdf(vk::Context& ctx, const std::filesystem::path& path)
{
    ...
    for (size_t gi = 0; gi < num_glyphs; ++gi) {
        ...
        if (g.bbx_w <= 0 || g.bbx_h <= 0) continue;
        if (g.codepoint == 0xFFFFFFFFu) continue; // unmapped
        ...
        for (int32_t row = 0; row < g.bbx_h; ++row) {
            for (int32_t col = 0; col < g.bbx_w; ++col) {
                if (!bdf::sample(g, col, row)) continue;
                ...
                if (static_cast<uint32_t>(px) >= atlas_w || static_cast<uint32_t>(py) >= atlas_h) continue;
...
std::expected<Font, Error> Font::load_pcf(
    vk::Context& ctx,
    const std::filesystem::path& path)
{
    auto buf_result = pcf::read_file(path);
    ...
    for (size_t gi = 0; gi < num_glyphs; ++gi) {
        ...
        if (ink_w <= 0 || ink_h <= 0) continue;

        gb.ink_w = static_cast<uint32_t>(ink_w);
        gb.ink_h = static_cast<uint32_t>(ink_h);
        uint32_t row_stride = (((gb.ink_w + 7) / 8) + binfo.pad_bytes - 1)
    ...
    uint32_t row_height = max_h + 1, rows_needed = 1;
    {
        ...
        for (size_t gi = 0; gi < num_glyphs; ++gi) {
            ...
            if (w == 0) continue;
            if (cxp + w + 1 > atlas_w) { cxp = 0; ++rows_needed; }
            cxp += w + 1;
        }
    }
    uint32_t atlas_h = 1;
    while (atlas_h < rows_needed * row_height) atlas_h *= 2;
    ...
    for (size_t gi = 0; gi < num_glyphs; ++gi) {
        auto& gb = bitmaps[gi];
        if (gb.ink_w == 0 || gb.ink_h == 0) continue;
        if (cx + gb.ink_w + 1 > atlas_w) { cx = 0; cy += row_height; }
        ...
        gb.metrics.uv = Rect{
            .x = static_cast<float>(cx) / aw, .y = static_cast<float>(cy) / ah,
            .w = static_cast<float>(gb.ink_w) / aw, .h = static_cast<float>(gb.ink_h) / ah,
        };
        cx += gb.ink_w + 1;
    }
...
std::expected<Font, Error> Font::load_bdf(
    vk::Context& ctx,
    const std::filesystem::path& path)
{
    auto parsed = bdf::parse(path);
    ...
    for (size_t gi = 0; gi < num_glyphs; ++gi) {
        ...
        if (g.bbx_w <= 0 || g.bbx_h <= 0 || g.codepoint == 0xFFFFFFFFu) continue;
        ...
        for (int32_t row = 0; row < g.bbx_h; ++row) {
            for (int32_t col = 0; col < g.bbx_w; ++col) {
                ...
                gb.pixels[dst+0] = gb.pixels[dst+1] = gb.pixels[dst+2] = gb.pixels[dst+3] = 255;
            }
        }
        total_width += gb.ink_w + 1;
        max_h = std::max(max_h, gb.ink_h);
...
std::expected<Font, Error> Font::from_atlas(
    Texture atlas,
    std::unordered_map<uint32_t, GlyphMetrics> glyphs,
    uint32_t pixel_size,
    float line_height,
...
float          Font::ascender()   const { return impl_->ascender; }
