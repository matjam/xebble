...
struct SpriteSheet::Impl {
    ...
    Impl(Texture tex, uint32_t tw, uint32_t th) : texture(std::move(tex)), tile_w(tw), tile_h(th) {
        cols = texture.width() / tile_w;
        rows = texture.height() / tile_h;
    }
};

std::expected<SpriteSheet, Error> SpriteSheet::load(vk::Context& ctx,
...
std::expected<SpriteSheet, Error> SpriteSheet::from_texture(Texture texture, uint32_t tile_width,
...
Rect SpriteSheet::region(uint32_t col, uint32_t row) const {
    return calculate_region(impl_->texture.width(), impl_->texture.height(), impl_->tile_w,
                            impl_->tile_h, col, row);
}

Rect SpriteSheet::calculate_region(uint32_t sheet_width, uint32_t sheet_height, uint32_t tile_width,
...
uint32_t SpriteSheet::tile_width() const {
    return impl_->tile_w;
}
uint32_t SpriteSheet::tile_height() const {
    return impl_->tile_h;
}
