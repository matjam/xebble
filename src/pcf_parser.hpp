...
struct Reader {
    ...
    uint16_t u16() noexcept {
        ...
        pos += 2;
        return msb ? static_cast<uint16_t>((a << 8) | b) : static_cast<uint16_t>((b << 8) | a);
    }
    ...
    uint32_t u32() noexcept {
        ...
        pos += 4;
        return msb ? (a << 24) | (b << 16) | (c << 8) | d : (d << 24) | (c << 16) | (b << 8) | a;
    }
...
read_file(const std::filesystem::path& path) {
    ...
    if (!f)
        return std::unexpected(xebble::Error{"PCF: cannot open " + path.string()});
    auto sz = static_cast<size_t>(f.tellg());
    f.seekg(0);
    std::vector<uint8_t> buf(sz);
    f.read(reinterpret_cast<char*>(buf.data()), static_cast<std::streamsize>(sz));
    if (!f)
        return std::unexpected(xebble::Error{"PCF: read error on " + path.string()});
...
inline std::expected<BitmapInfo, xebble::Error> read_bitmap_info(Reader& r, const TocEntry& entry,
    ...
    for (auto& o : info.offsets)
        o = r.u32();

    for (int i = 0; i < 4; ++i)
        r.u32(); // bitmapSizes[4] — skip
...
read_encodings(Reader& r, const TocEntry& entry, size_t num_glyphs) {
    ...
    for (auto& idx : indices)
        idx = static_cast<uint16_t>(r.u16());
