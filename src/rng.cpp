...
int32_t Rng::roll(std::string_view expr) {
    ...
    if (p < end && *p != 'd' && *p != 'D') {
        if (!parse_uint(count) || count < 1)
            throw std::invalid_argument("Rng::roll: invalid die count in '" +
                                        std::string(expr) + '\'');
    }

    // 'd' separator.
    if (p >= end || (*p != 'd' && *p != 'D'))
        throw std::invalid_argument("Rng::roll: missing 'd' in '" +
                                    std::string(expr) + '\'');
    ++p;
    ...
    if (!parse_uint(faces) || faces < 1)
        throw std::invalid_argument("Rng::roll: invalid face count in '" +
                                    std::string(expr) + '\'');
    ...
    if (p < end) {
        if (*p != '+' && *p != '-')
            throw std::invalid_argument("Rng::roll: unexpected character '" +
                                        std::string(1, *p) + "' in '" +
                                        std::string(expr) + '\'');
        char sign = *p++;
        int32_t mag = 0;
    ...
    // Trailing garbage.
    if (p != end)
        throw std::invalid_argument("Rng::roll: trailing characters in '" +
                                    std::string(expr) + '\'');
